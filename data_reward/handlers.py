...
class HHrlhfHandler(BaseHandler):
    ...
    def map(self, example):
        ...
        return inp.strip(), out.strip(), r


class FeedbackCollectionHandler(BaseHandler):
...
class PKUSafeHandler(BaseHandler):
...
class MetaMathDPOHandler(BaseHandler):
...
class AceCodePairHandler(BaseHandler):
...
class HelpSteer3PreferenceHandler(BaseHandler):
...
class UltraFeedbackBinarizedHandler(BaseHandler):
...
class DeepseekORMHandler(BaseHandler):
...
class MistralORMHandler(BaseHandler):
...
class AceMathRMHandler(BaseHandler):
    # TODO: deduplicate inputs
...
class UltraInteractHandler(BaseHandler):
...
class SkyworkRewardPreferenceHandler(BaseHandler):
